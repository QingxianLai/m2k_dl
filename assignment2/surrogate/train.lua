...
opt = lapp[[
   ...
   --model                    (default surrogate)     model name
...
provider = torch.load('extra.t7')
provider.unlabelData.data = provider.unlabelData.data:float()
confusion = optim.ConfusionMatrix(4096)
...
function train()
    ...
    local indices = torch.randperm(provider.unlabelData.data:size(1)):long():split(opt.batchSize)
    ...
    for t,v in ipairs(indices) do
        ...
        local inputs = provider.unlabelData.data:index(1,v)
        targets:copy(provider.unlabelData.labels:index(1,v))
