-- This module has been modified to return the cluster labels and the true count for each cluster
-- the original code is back up in kmeans.lua.backup
...
function unsup.kmeans(x, k, niter, batchsize, callback, verbose)
   ...
   for i = 1,niter do
      ...
      local loss = 0
      l = torch.FloatTensor(nsamples):zero()

      -- process batch
      for i = 1,nsamples,batchsize do
         -- indices
         local lasti = math.min(i+batchsize-1,nsamples) -- last index of this batch
         local m = lasti - i + 1 -- number of samples in this batch
         ...
         l[{{i,lasti}}]:copy(labels:reshape(m))
   ...
   -- done
   return centroids:reshape(k_size),l,totalcounts
end
