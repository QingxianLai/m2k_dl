...
function augmentTrain(trainData, n_fold)
    ...
    local newTrain = {
        data = augData,
        labels = auglabel,
        size = function() return size * (n_fold + 1) end
    }
    return newTrain
end
...
function Provider:__init(full)
    ...
    -- load extra data 
    local l_extra = torch.load('./l_extra.t7')
    ...
    local d = torch.FloatTensor(self.trainData:size() + #l_extra.data, channel, height, width)
    local l = torch.FloatTensor(self.trainData:size() + #l_extra.data)
    d[{{1,self.trainData.size()},{},{},{}}]:copy(self.trainData.data)
    l[{{1,self.trainData.size()}}]:copy(self.trainData.labels)
    for i = 1,#l_extra.data do
        d[i+self.trainData.size()]:copy(l_extra.data[i])
        l[i+self.trainData.size()] = l_extra.labels[i]
    end
    
    local tmp_size = self.trainData.size()

    self.trainData.data = d
    self.trainData.labels = l
    self.trainData.size = function() return tmp_size + #l_extra.data end
