...
all_size = trsize
trsize = all_size * 0.8
tesize = all_size - trsize


trainData = {
   data = loaded.data[{{1,trsize},{},{},{}}],
   labels = loaded.labels[{{1,trsize}}],
   size = function() return trsize end   -- TODO why use function
}

--loaded = torch.load(test_file, 'ascii')

testData = {
   data = loaded.data[{{trsize+1,all_size},{},{},{}}],
   labels = loaded.labels[{{trsize+1,all_size}}],
...
-- As we are using MNIST which only has one channel, ignore the above paragraph --You should say this earlier!
